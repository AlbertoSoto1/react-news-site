...
export default class Principal extends Component{
    ...
    getRecientes(){
        ...
        var query = noticiasRef.where("categoria","==",this.props.match.params.cat).orderBy("timeStamp","desc").limit(6).get().then((querySnapshot) => {
    ...
    getLikeadas(){
        var vacio = [];
        var noticiasRef = db.collection("noticias");
        var query = noticiasRef.where("categoria","==",this.props.match.params.cat).orderBy("likes","desc").limit(9).get().then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
                var documento = new Object();
                documento.id = doc.id;
                documento.data = doc.data();
